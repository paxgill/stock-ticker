  ...
  wrap.innerHTML = `
    <div class="market-summary-card">
      ...
      <div class="ms-text-wrap">
        <div class="ms-label" id="ms-label">DAILY MARKET SUMMARY</div>
        <div class="ms-paragraph" id="ms-paragraph">${esc(data.summary)}</div>
        <div class="ms-timestamp" id="ms-attrib">Generated ${time}</div>
      </div>
    </div>`;
  if (S.aiEnabled) loadMorningBriefing(false);
}

// Morning briefing — swaps the template summary for a Claude-written note
async function loadMorningBriefing(force) {
  const label = document.getElementById('ms-label');
  const para = document.getElementById('ms-paragraph');
  const attrib = document.getElementById('ms-attrib');
  if (!para) return;
  if (force) para.innerHTML = `<span class="ai-loading"><span class="loading-spinner" style="width:12px;height:12px;border-width:1px"></span> Asking Claude…</span>`;
  try {
    const r = await api.get(`/api/ai/briefing${force ? '?force=1' : ''}`);
    if (!r.enabled || !r.summary) return;          // keep the template text
    if (label) label.textContent = 'MORNING BRIEFING';
    para.innerHTML = `<p>${aiText(r.summary)}</p>`;
    if (attrib) attrib.innerHTML = aiAttribution('loadMorningBriefing(true)');
  } catch (e) { /* keep template silently */ }
}
...
function openSignalDetail(symbol) {
  ...
  body.innerHTML = `
    ...
    <div class="sd-section" id="sd-ai-slot" style="${S.aiEnabled ? '' : 'display:none'}"></div>
  ...
  if (S.aiEnabled) loadSignalExplanation(symbol, false);
}

async function loadSignalExplanation(symbol, force) {
  const slot = document.getElementById('sd-ai-slot');
  if (!slot) return;
  slot.innerHTML = `<div class="sd-section-title">Plain-English explanation</div>
    <div class="ai-loading"><span class="loading-spinner" style="width:12px;height:12px;border-width:1px"></span> Asking Claude…</div>`;
  try {
    const r = await api.get(`/api/ai/signal-explanation/${encodeURIComponent(symbol)}${force ? '?force=1' : ''}`);
    if (!r.enabled) { slot.style.display = 'none'; return; }
    if (!r.explanation) { slot.innerHTML = `<div class="sd-section-title">Plain-English explanation</div><p class="muted">Explanation unavailable right now.</p>`; return; }
    slot.innerHTML = `<div class="sd-section-title">Plain-English explanation</div>
      <div class="ai-block"><p>${aiText(r.explanation)}</p></div>
      ${aiAttribution(`loadSignalExplanation('${symbol}', true)`)}`;
  } catch (e) {
    slot.innerHTML = `<div class="sd-section-title">Plain-English explanation</div><p class="muted">Could not reach the AI service.</p>`;
  }
}
...
// ════════════════════════════════════════════════════════════════
// CLAUDE FABLE 5 AI LAYER (Phase 3)
// ════════════════════════════════════════════════════════════════
async function loadAiStatus() {
  try { const r = await api.get('/api/ai/status'); S.aiEnabled = !!r.enabled; }
  catch (e) { S.aiEnabled = false; }
}

// "Generated by Claude" footer with a regenerate button
function aiAttribution(onRegen) {
  return `<div class="ai-attrib"><span class="ai-tag">✦ Generated by Claude</span>
    <button class="ai-regen" onclick="${onRegen}">↻ Regenerate</button></div>`;
}
// Render multiline AI text safely (escape, keep paragraph breaks)
function aiText(t) {
  return esc(t || '').replace(/\n{2,}/g, '</p><p>').replace(/\n/g, '<br>');
}

// Show/hide AI-only affordances based on whether the key is configured
function applyAiVisibility() {
  const bar = document.getElementById('ai-query-bar');
  if (bar) bar.style.display = S.aiEnabled ? '' : 'none';
}

async function runWatchlistQuery() {
  const input = document.getElementById('ai-query-input');
  const out = document.getElementById('ai-query-result');
  const q = (input.value || '').trim();
  if (!q) return;
  out.style.display = '';
  out.innerHTML = `<div class="ai-loading"><span class="loading-spinner" style="width:12px;height:12px;border-width:1px"></span> Claude is checking your watchlist…</div>`;
  try {
    const r = await api.post('/api/ai/watchlist-query', { query: q });
    if (!r.enabled) { out.style.display = 'none'; return; }
    const matches = (r.result && r.result.matches) || [];
    if (!matches.length) {
      out.innerHTML = `<div class="ai-block"><p class="muted">No watchlist names match that.</p></div>`;
      return;
    }
    out.innerHTML = `
      <div class="ai-block">
        <div class="sd-section-title">Matches for "${esc(q)}"</div>
        <div class="ai-match-grid">
          ${matches.map(m => `
            <div class="ai-match" onclick="openSignalDetail('${esc(m.symbol)}')">
              <div class="ai-match-sym">${esc(m.symbol)}</div>
              <div class="ai-match-reason">${esc(m.reason)}</div>
            </div>`).join('')}
        </div>
        <div class="sd-disclaimer">${esc((r.result && r.result.disclaimer) || '')}</div>
      </div>
      ${aiAttribution('runWatchlistQuery()')}`;
  } catch (e) {
    out.innerHTML = `<div class="ai-block"><p class="muted">Could not reach the AI service.</p></div>`;
  }
}
...
function renderBacktestResult(bt) {
  ...
  const aiBtn = S.aiEnabled
    ? `<div class="bt-ai-actions"><button class="btn-sm" onclick="runBacktestPostmortem(false)">✦ Explain these results</button></div>
       <div id="bt-postmortem"></div>`
    : '';
...
async function runBacktestPostmortem(force) {
  const slot = document.getElementById('bt-postmortem');
  const bt = S.lastBacktest;
  if (!slot || !bt) return;
  slot.innerHTML = `<div class="ai-loading"><span class="loading-spinner" style="width:12px;height:12px;border-width:1px"></span> Claude is reading the results…</div>`;
  try {
    const r = await api.post('/api/ai/backtest-postmortem', { stats: bt.stats, trades: bt.trades, force: !!force });
    if (!r.enabled || !r.explanation) { slot.innerHTML = ''; return; }
    slot.innerHTML = `<div class="ai-block"><p>${aiText(r.explanation)}</p></div>${aiAttribution('runBacktestPostmortem(true)')}`;
  } catch (e) {
...
function renderJournal() {
  ...
  // Reveal the AI review button only when the key is configured and trades exist
  const aiBtn = document.getElementById('journal-ai-btn');
  if (aiBtn) aiBtn.style.display = (S.aiEnabled && S.trades.length) ? '' : 'none';
...
async function runJournalReview(force) {
  const box = document.getElementById('journal-ai-review');
  if (!box) return;
  box.style.display = '';
  box.innerHTML = `<div class="ai-loading"><span class="loading-spinner" style="width:12px;height:12px;border-width:1px"></span> Claude is reviewing your trades…</div>`;
  try {
    const r = await api.post('/api/ai/journal-review', { force: !!force });
    if (!r.enabled) { box.style.display = 'none'; return; }
    if (r.message) { box.innerHTML = `<p class="muted">${esc(r.message)}</p>`; return; }
    const res = r.result;
    if (!res || !res.observations || !res.observations.length) {
      box.innerHTML = `<p class="muted">No clear patterns to surface yet.</p>`; return;
    }
    box.innerHTML = `
      <div class="ai-block">
        <div class="sd-section-title">Patterns to consider</div>
        <ul class="ai-bullets">${res.observations.map(o => `<li>${esc(o)}</li>`).join('')}</ul>
        <div class="sd-disclaimer">${esc(res.disclaimer || '')}</div>
      </div>
      ${aiAttribution('runJournalReview(true)')}`;
  } catch (e) {
    box.innerHTML = `<p class="muted">Could not reach the AI service.</p>`;
  }
}
